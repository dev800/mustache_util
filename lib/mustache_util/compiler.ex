defmodule MustacheUtil.Compiler do
  def compile(source, bindings, options) do
    tokens = MustacheUtil.Tokenizer.tokenize(source)
    partials = options[:partials] || []

    partials =
      Enum.map(partials, fn {k, partial} -> {k, MustacheUtil.Tokenizer.tokenize(partial)} end)

    build(tokens, bindings, partials: partials, root: bindings) |> List.flatten() |> Enum.join()
  end

  def escape(value) do
    value |> MustacheUtil.Tool.to_binary() |> MustacheUtil.Tool.escape_html()
  end
  ...
  def get_value(bindings, name, _root) when is_list(bindings) and is_atom(name),
    do: bindings[name]
  ...
  def get_value(bindings, name, root) when is_tuple(bindings) and is_atom(name),
    do: get_value(Keyword.new([bindings]), name, root)
  ...
  def build([{tag, name} | rest], bindings, opts) when tag in [:variable, :dot, :dotted_name] do
    [get_value(bindings, name, opts[:root]) |> check_lambda |> escape] ++
      build(rest, bindings, opts)
  end

  def build([{tag, name} | rest], bindings, opts)
      when tag in [:unescaped_variable, :unescaped_dot, :unescaped_dotted_name] do
    [get_value(bindings, name, opts[:root]) |> check_lambda |> to_string] ++
      build(rest, bindings, opts)
  end
  ...
  def build([{tag, name} | rest], bindings, opts)
      when tag in [:inverted_section, :dotted_name_inverted_section] do
    bind = get_value(bindings, name, opts[:root])
    idx = Enum.find_index(rest, fn e -> {:end_section, name} == e end)
    elements = Enum.take(rest, idx)
