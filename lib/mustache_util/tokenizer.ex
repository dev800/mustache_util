defmodule MustacheUtil.Tokenizer do
  @moduledoc false
  ...
  defp scan('{{{' ++ t, scanned, buf, :in_text),
    do: scan(t, scanned ++ [parse_text(buf)], [], :in_unescaped_var)

  defp scan('}}}' ++ t, scanned, buf, :in_unescaped_var),
    do: scan(t, scanned ++ [parse_variable(buf, :unescaped)], [], :in_text)

  defp scan('{{' ++ t, scanned, buf, :in_text),
    do: scan(t, scanned ++ [parse_text(buf)], [], :in_tag)

  defp scan('}}' ++ t, scanned, buf, :in_tag),
    do: scan(t, scanned ++ [parse_tag(strip(buf))], [], :in_text)
